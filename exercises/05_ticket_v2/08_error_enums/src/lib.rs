    ...
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(err) => {
           match err {
               ...
               TicketNewError::DescriptionError { err_msg } => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err( TicketNewError::TitleError { err_msg: ("Title cannot be empty".to_string()) });
        }
        if title.len() > 50 {
            return Err( TicketNewError::TitleError { err_msg: ("Title cannot be longer than 50 bytes".to_string()) });
        }
        if description.is_empty() {
            return Err( TicketNewError::DescriptionError { err_msg: ("Description cannot be empty".to_string()) });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { err_msg: ("Description cannot be longer than 500 bytes".to_string()) });
        }
